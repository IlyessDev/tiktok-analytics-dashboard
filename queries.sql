...
-- Toutes les vidéos triées par vues décroissantes
SELECT *
FROM videos
ORDER BY "Vues" DESC
...
-- Catégorisation des vidéos (Hit / Moyen / Flop)
SELECT "Titre", "Vues",
  CASE
...
-- Comptage et moyenne par catégorie de performance
...
-- Catégorie durée (< 20s / 20-40s / 40-60s / > 60s)
SELECT "Titre", "Duree",
  CASE
...
-- Stats par casting avec taux d'engagement moyen
...
-- Toutes les vidéos avec rang global + moyenne du casting + écart
WITH video_stats AS (
...
-- Rang par vues - global et par casting
...
-- Momentum - évolution des vues par rapport à la vidéo précédente
...
-- Flops consécutifs avant chaque hit
