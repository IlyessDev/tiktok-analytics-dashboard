...
--  Colonnes : id, Titre, Casting, Lieu, Duree, Date,
--             Vues, Likes, Partages, Notes
-- ============================================================
...
-- ============================================================
--  MODULE 2 — SQL ANALYTIQUE
-- ============================================================

-- 2.1 Taux d'engagement par vidéo
--     Règle : (Likes + Partages) / Vues * 100
--     ::float évite la division entière en PostgreSQL
SELECT
  "Titre",
  "Vues",
  "Likes",
...
--     Utilise une sous-requête pour grouper sur le résultat du CASE
SELECT
  performance,
  COUNT(*)           AS nb_videos,
  ROUND(AVG("Vues")) AS moyenne_vues
...
--     Réplique l'analyse "séquence Flop → Hit" du dashboard JS
...
hits_avec_precedent AS (
  SELECT
    "Titre", "Vues", numero,
    LAG(numero) OVER (ORDER BY numero) AS numero_hit_precedent
...
-- 4.4 Vélocité — délai entre chaque vidéo
SELECT
  "Titre",
  "Date",
  "Vues",
  LAG("Date") OVER (ORDER BY "Date")                          AS date_precedente,
  ("Date" - LAG("Date") OVER (ORDER BY "Date"))               AS delai,
  CASE
    WHEN ("Date" - LAG("Date") OVER (ORDER BY "Date")) >= 25  THEN '⚠️ Longue pause'
...
-- ============================================================
--  BONUS — REQUÊTE TABLEAU DE BORD COMPLET
--  Synthèse de toutes les métriques en une seule requête
-- ============================================================

WITH base AS (
  SELECT
    "Titre", "Casting", "Lieu", "Duree", "Date", "Vues", "Likes", "Partages",
    ROW_NUMBER() OVER (ORDER BY "Date")           AS numero,
    RANK()       OVER (ORDER BY "Vues" DESC)      AS rang_global,
    LAG("Vues")  OVER (ORDER BY "Date")           AS vues_precedente,
    ROUND(AVG("Vues") OVER (PARTITION BY "Casting")) AS moy_casting,
